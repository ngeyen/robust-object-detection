...
MODEL_PATH = "models/pretrained/ssdlite_mobiledet_edgetpu_320x320_coco_2020_05_19/fp32/model.tflite"
VEHICLE_CLASS_IDS = [2, 3, 4, 5, 6, 7, 8, 9]  # COCO vehicle categories (1-indexed in TF)

def load_model():
    """Load pre-trained SSDLite-MobileDet from local .tflite file."""
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(f"Model file not found at {MODEL_PATH}. Please ensure it’s extracted correctly.")
    interpreter = tf.lite.Interpreter(model_path=MODEL_PATH)
    interpreter.allocate_tensors()
    return interpreter

def run(image_path, output_dir="experiments/baseline_tests/ssdlite_mobiledet", gt_annotations=None):
    """Run inference on a single image and filter for vehicles."""
    ...
    input_tensor = cv2.resize(img, (320, 320))  # SSDLite-MobileDet uses 320x320
    ...
    boxes = interpreter.get_tensor(output_details[0]['index'])[0]  # [y_min, x_min, y_max, x_max]
    classes = interpreter.get_tensor(output_details[1]['index'])[0].astype(int)  # Class IDs
