...
KITCHEN_CLASS_IDS = [44, 46, 47, 48, 49, 50, 51]
...
def run(image_path, output_dir="experiments/baseline_tests/yolov5_nano_kitchen", gt_annotations=None):
    model = load_model()
    results = model(image_path)
    preds = results.xyxy[0]

    # Move KITCHEN_CLASS_IDS to the same device as preds
    device = preds.device
    kitchen_class_tensor = torch.tensor(KITCHEN_CLASS_IDS).to(device)

    kitchen_mask = torch.isin(preds[:, 5], kitchen_class_tensor)
    kitchen_preds = preds[kitchen_mask]

    print("Kitchen Object Bounding Box Tensor:")
    print(kitchen_preds)
    ...
    return kitchen_preds

if __name__ == "__main__":
    sample_image = "data/processed/coco_occluded_kitchen/000000001424.jpg"
    # For standalone testing, load sample GT (remove in production)
    with open("data/annotations/coco_occluded_kitchen.json", 'r') as f:
        gt_data = json.load(f)
